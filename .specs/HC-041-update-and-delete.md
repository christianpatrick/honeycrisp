...
Reminders update takes `title`, `due`, `notes`, `list` (moves it), `completed` (false reopens a reminder that was completed by mistake), and `url`, which create accepts too. The url lands in EventKit's URL field so links, like a note link, travel with the reminder; some Reminders versions do not display that field in the app, the tool descriptions say so, and a link that must be visible belongs in notes as well. Calendar update takes `title`, `start`, `end`, `all_day`, `calendar` (moves it), `location`, `notes`, and `url`, which create accepts too and Calendar shows on the event; moving only `start` keeps the event's duration by shifting the end, and an `end` at or before the final start refuses with a sentence. Contacts update takes the create fields. Mail update takes `read` and `flagged` booleans, at least one required, over `message_id` or a whole `thread_id`.
