...
- How we touch Apple apps, in strict order of preference:
  ...
  3. In-process Apple events, only for the few writes that have no framework or store-safe path: composing a Mail draft, sending mail, sending a message, and changing Mail message state (read, flagged, and delete to the Trash, HC-041). These are raw Apple events sent from Swift under the Automation permission. We never spawn osascript and we never compile or run AppleScript source; both are slow and brittle, and the whole point of Honeycrisp is to not be that.
...
- Writes whose effect leaves the Mac (send mail and send message) always require notification approval even when their switch is on. Local writes (create, update, complete, and delete a reminder, create a contact and update or delete a card, create, update, and delete a calendar event, save a Mail draft, mark a conversation read, mark mail read, flag or unflag mail, and move mail to the Trash) run without a prompt when their switch is on. There is no Messages draft: iMessage has no draft concept, so a tool named draft that actually sent would lie about its effect (removed in HC-025).
- Updates and deletes (HC-041) keep the approval invariant: approval means an effect leaves the Mac, and nothing an update or delete touches does. Deletes ship off by default like every write and land in the audit log with what was deleted. Updates are partial, with an empty string clearing a clearable field. Messages has no update or delete because no sanctioned path exists: chat.db is read-only by rule and the Messages dictionary cannot edit or remove a message.
...
- The action catalog is twenty-eight actions across five apps. The typed catalog in code (ActionCatalog) is the single source of truth.
