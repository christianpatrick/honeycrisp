...
- The action catalog comes from the catalog spec plus messages mark read, mail mark read, and the calendar trio, minus the unreal messages draft, nineteen actions total across five apps. After HC-002 the catalog in code is the single source of truth.
...
- Brand assets are never committed, and the whole `assets/` tree is gitignored (not one file in it is tracked). The repo carries only the shippable artwork committed, under `assets/`, and builds and packaging depend on `assets/` alone. Deliveries unzip locally under `assets/` for reference: as of 2026-06-09, `the spec ` is the menu bar app spec ("Menu bar app.zip") whose the catalog spec is the source of the action catalog and whose the panel spec, the onboarding spec, and the canvas spec define the panel, first run, and approval flow; `the brand spec` is the full brand ("Honeycrisp brand spec") that locks the wordmark (Sora semibold, -0.03em tracking, red period) and ships the Calendar icon. Specs cite these files by name for provenance; those citations describe the local artifact, and a fresh clone never needs them to build.
