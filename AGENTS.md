...
- How we touch Apple apps, in strict order of preference:
  ...
  2. Read-only database and file access when no framework exists: chat.db SQLite for Messages reads, Mail's Envelope Index SQLite plus .emlx files for Mail reads, NoteStore.sqlite for Notes reads (the note text is a gzipped protobuf; the plain text sits at field path 2, 3, 2). Open read-only. We never write to a data store another app owns, ever: no chat.db writes, no Envelope Index writes, no .emlx injection, no NoteStore writes. If Honeycrisp needs an index of its own, it builds one inside its own Application Support folder.
  3. In-process Apple events, only for the few writes that have no framework or store-safe path: composing a Mail draft, sending mail, sending a message, creating a note, appending to a note. These are raw Apple events sent from Swift under the Automation permission. We never spawn osascript and we never compile or run AppleScript source; both are slow and brittle, and the whole point of Honeycrisp is to not be that.
...
- Writes whose effect leaves the Mac (send mail and send message) always require notification approval even when their switch is on. Local writes (create reminder, complete reminder, create contact, create calendar event, save a Mail draft, mark a conversation read, mark mail read, create a note, append to a note) run without a prompt when their switch is on. There is no Messages draft: iMessage has no draft concept, so a tool named draft that actually sent would lie about its effect (removed in HC-025).
- Notes has no scriptable share, so note links never depend on sharing (HC-040): notes_link builds the applenotes:note/uuid URL from the store's ZIDENTIFIER, the same links Notes writes for its own note-to-note references, and they open on every device signed into the same iCloud account. The signed-in address itself is read locally from MobileMeAccounts.plist for clients that want the manual icloud.com share flow. Appending rewrites the whole body over Apple events, and a rewrite drops embedded attachments, so notes_append refuses locked notes and attachment carriers; there is no delete and no body replace in the toolset.
...
- The action catalog is twenty-five actions across six apps. The typed catalog in code (ActionCatalog) is the single source of truth.
- Not every tool is a catalog row. Container discovery tools (mail_mailboxes, reminders_lists, calendar_calendars, notes_folders) are registry aliases: each rides the permission switch of an existing read action (mail.search, reminders.list, calendar.list, notes.search) but executes and audits its own operation. A new alias needs no catalog, permission, or UI change, only a registry entry.
...
- website/ is the marketing site (HC-039): a self-contained Astro static build of the letter design, deployed to GitHub Pages at https://honeycrisp.app (the custom domain in the repository's Pages settings) by .github/workflows/website.yml. The workflow is path-filtered to website/** and assets/**, builds and tests on pull requests, and deploys only from main. Pages must be set to deploy from GitHub Actions in the repository settings; the workflow attempts to enable that itself.
...
- Fonts (Caveat, Newsreader, Sora) are vendored OFL woff2 files in website/src/assets/fonts with their license, so the site makes no Google Fonts requests at runtime. Brand SVGs are not copied: the page imports the icon and app icons straight from assets/, the same files the README and app use, and the Pages workflow triggers on assets/** so artwork changes redeploy the site.
